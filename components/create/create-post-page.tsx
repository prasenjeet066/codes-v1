...
export default function CreatePostPage({ user }: CreatePostPageProps) {
  ...
  const handlePost = async (pollData?: PollData) => {
    if (!content.trim() && totalMediaCount === 0 && !pollData) {
    ...
    try {
      ...
      if (pollData) {
        postPayload.poll_data = pollData
      }
        ...
        <div className="mb-4">
          {showPollCreator ? (
            <PollCreator
              ...
              onSubmit={(pollData) => handlePost(pollData)}
            />
          ) : (
            <div
            ...
            <CardContent className="pt-2 p-4">
              <div className="grid grid-cols-2 gap-2 max-h-96 overflow-y-auto">
                {gifs.map((gif) => (
                  ...
                  >
                    <img src={gif.images?.original?.url} alt="Giphy" className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
